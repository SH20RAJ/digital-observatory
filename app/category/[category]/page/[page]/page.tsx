...
export function generateStaticParams(){
  ...
  for(const category of getCategories()){
    ...
    for(const page of getPageNumbers(posts.length))if(page>1)output.push({category:categorySlug(category),page:String(page)});
  }
  return output;
}
export async function generateMetadata({params}:{params:Promise<{category:string;page:string}>}):Promise<Metadata>{
  ...
  if(!posts.length||!Number.isInteger(current)||current<2||current>total)return {};
  return {title:posts[0].category+" — Page "+current,description:"Page "+current+" of the "+posts[0].category+" archive.",alternates:{canonical:"/category/"+category+"/page/"+current}};
}
export default async function CategoryPageNumber({params}:{params:Promise<{category:string;page:string}>}){
  const {category,page}=await params; const posts=getAllPosts().filter(post=>categorySlug(post.category)===category); const current=Number(page); const total=getPageCount(posts.length);
  if(!posts.length||!Number.isInteger(current)||current<2||current>total)notFound();
