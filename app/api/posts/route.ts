...
export function GET(request: Request) {
  ...
  const posts = (q ? findPosts(q) : getAllPosts()).map((post) => ({
    ...
    publishedAt: post.publishedAt, updatedAt: post.updatedAt, category: post.category, tags: post.tags,
    author: post.author, readingTime: post.readingTime, url: new URL("/blog/" + post.slug, request.url).toString()
  }));
