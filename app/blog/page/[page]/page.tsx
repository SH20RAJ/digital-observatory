...
export function generateStaticParams(){return getPageNumbers(getAllPosts().length).filter(page=>page>1).map(page=>({page:String(page)}));}
export async function generateMetadata({params}:{params:Promise<{page:string}>}):Promise<Metadata>{
  const {page}=await params; const current=Number(page); const total=getPageCount(getAllPosts().length);
  if(!Number.isInteger(current)||current<2||current>total)return {};
  return {title:"Journal — Page "+current,description:"Page "+current+" of the Digital Observatory journal archive.",alternates:{canonical:"/blog/page/"+current}};
}
