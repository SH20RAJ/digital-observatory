...
export function generateStaticParams(){
  ...
  for(const tag of getTags()){
    ...
    for(const page of getPageNumbers(posts.length))if(page>1)output.push({tag:tagSlug(tag),page:String(page)});
  }
  return output;
}
export async function generateMetadata({params}:{params:Promise<{tag:string;page:string}>}):Promise<Metadata>{
  ...
  return{title:"#"+tag+" — Page "+current,description:"Page "+current+" of the "+tag+" tag archive.",alternates:{canonical:"/tag/"+tag+"/page/"+current}};
}
