...
export async function generateMetadata({params}:{params:Promise<{tag:string}>}):Promise<Metadata>{
  ...
  const title="#"+tag;
  const description="Digital Observatory articles tagged #"+tag+".";
  const url="/tag/"+tag;
...
export default async function TagPage({params}:{params:Promise<{tag:string}>}){
  ...
  const crumbs=[{name:"Home",path:"/"},{name:"Tags",path:"/blog"},{name:"#"+tag,path:"/tag/"+tag}];
  return <div className="page"><div className="shell">
    <Breadcrumbs items={crumbs.map((c)=>({name:c.name,href:c.path}))}/>
    <div className="page-heading"><p className="eyebrow">Tag archive</p><h1>#{tag}</h1><p>{posts.length} published observation{posts.length===1?"":"s"} in this thread.</p></div>
    ...
    <script type="application/ld+json" dangerouslySetInnerHTML={jsonLd(collectionJsonLd({name:"#"+tag,description:posts.length+" published observations tagged "+tag,url:"/tag/"+tag,posts:pageItems}))}/>
  </div></div>;
}
