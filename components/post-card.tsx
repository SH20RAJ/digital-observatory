  ...
  return (
    <article className={featured ? "post-card post-card-featured" : "post-card"}>
      <Link href={"/blog/" + post.slug} className="post-card-media" aria-label={post.title}>
        <Image src={poster} alt={post.coverAlt || ""} fill sizes={featured ? "(max-width: 920px) 100vw, 70vw" : "(max-width: 640px) 100vw, (max-width: 920px) 50vw, 33vw"} priority={featured} className="card-image" />
